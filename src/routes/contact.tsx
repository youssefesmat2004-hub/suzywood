...
function Contact() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(false);
    if (error) toast.error(t("pages.contactCouldntSend", "Couldn't send"), { description: error.message });
    else { toast.success(t("pages.contactMessageSent", "Message sent")); (e.target as HTMLFormElement).reset(); }
  };

  return (
    <Layout>
      <section className="container mx-auto px-6 lg:px-10 pt-16 pb-24 max-w-5xl">
        <p className="text-[11px] uppercase tracking-[0.28em] text-secondary mb-3">{t("pages.contactGetInTouch", "Get in touch")}</p>
        <h1 className="font-serif text-5xl md:text-6xl">{t("pages.contactHeading", "We'd love to hear about your space.")}</h1>

        <h2 className="sr-only">{t("pages.contactDetailsHeading", "Our Contact Details")}</h2>

        <div className="mt-16 grid lg:grid-cols-3 gap-10">
          <div className="space-y-6">
            {[
              { Icon: Phone, title: t("pages.contactPhoneWhatsapp", "Phone & WhatsApp"), body: "+20 109 631 3532", sub: t("pages.contactHours", "Sun–Thu, 10am–6pm"), href: "tel:+201096313532" },
          ...
          <form onSubmit={onSubmit} className="lg:col-span-2 bg-card border border-border rounded-2xl p-8 space-y-5">
            <div className="grid sm:grid-cols-2 gap-5">
              <div className="space-y-1"><Label htmlFor="name">{t("common.name", "Name")}</Label><Input id="name" name="name" required maxLength={100} /></div>
              <div className="space-y-1"><Label htmlFor="phone">{t("pages.contactPhoneOptional", "Phone (optional)")}</Label><Input id="phone" name="phone" type="tel" maxLength={30} /></div>
            </div>
