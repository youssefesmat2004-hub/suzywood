...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    IF v_has_mattress THEN
      ...
      END IF;
      -- Accept the mattress price sent by the client as long as it matches
      -- one of the configured tier prices for the category. This avoids
      -- failing checkout when the selected size label doesn't exactly match
      -- a category_sizes row (case, spacing, add-on suffix, custom size).
      ...
      ELSE
        -- Fallback: default to the small-tier price (cheaper of the two)
