...
ALTER TABLE public.categories
  ADD COLUMN IF NOT EXISTS mattress_addon_enabled boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS mattress_small_price numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS mattress_big_price numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS mattress_addon_note text;

ALTER TABLE public.category_sizes
  ADD COLUMN IF NOT EXISTS mattress_tier text;

ALTER TABLE public.category_sizes
  DROP CONSTRAINT IF EXISTS category_sizes_mattress_tier_check;
ALTER TABLE public.category_sizes
  ADD CONSTRAINT category_sizes_mattress_tier_check
  CHECK (mattress_tier IS NULL OR mattress_tier IN ('small', 'big'));

ALTER TABLE public.order_items
  ADD COLUMN IF NOT EXISTS mattress boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS mattress_price numeric NOT NULL DEFAULT 0;

-- Replace create_order_with_items to support mattress add-on
CREATE OR REPLACE FUNCTION public.create_order_with_items(
  ...
  _delivery_area text DEFAULT NULL::text,
  _order_size_type text DEFAULT 'big'::text
)
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    IF COALESCE(v_item->>'size', '') <> '' THEN
      SELECT pv.price INTO v_variant_price FROM public.product_variants pv
    ...
    IF v_has_mattress THEN
      ...
      END IF;
      v_mattress_tier := NULL;
      IF COALESCE(v_item->>'size', '') <> '' THEN
        SELECT cs.mattress_tier INTO v_mattress_tier
          FROM public.category_sizes cs
          WHERE cs.category_id = v_product_category
            AND lower(cs.label) = lower(v_item->>'size')
          LIMIT 1;
      END IF;
      IF v_mattress_tier IS NULL THEN
        RAISE EXCEPTION 'Mattress not available for selected size of %', v_item->>'product_name';
      END IF;
      v_mattress_price := CASE WHEN v_mattress_tier = 'small' THEN v_mattress_small ELSE v_mattress_big END;
    END IF;
